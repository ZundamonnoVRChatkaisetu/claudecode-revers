...
## phase 2: srcフォルダ内に復元したファイルのみで、実際に動作するように修正を行う。
 - [ ] 復元したファイルすべてを確認し、難読化した状態の変数がないか確認する
    - [ ] command-security.js部分的修正完了
    - [ ] command-security.jsの残りの難読化変数と未定義関数を修正する     
    - [ ] stats-telemetry.js (41個の難読化変数) を修正する
    - [ ] installation-manager.js (43個の難読化変数) を修正する
 - [ ] 復元したファイルすべてを確認し、難読化した状態の処理がないか確認する
 - [ ] srcフォルダ内のみで実際に動作するように修正を行う